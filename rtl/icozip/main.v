...
// check
	...
	// from
	localparam	RESET_ADDRESS = 32'h00000800;
	//
...
`include "builddate.v"
	...
	assign	        gpio_sel = (sio_sel)&&(sio_skip[ 2: 0] ==  3'b010_);
	assign	    pwrcount_sel = (sio_sel)&&(sio_skip[ 2: 0] ==  3'b011_);
	assign	     version_sel = (sio_sel)&&(sio_skip[ 2: 0] ==  3'b100_);
	wire	[2:0]	wb_skip; // bits 00000c20, sbaw=10
	assign	wb_skip = {
			wb_addr[ 9: 8],
			wb_addr[3]
		};
	assign	        uart_sel = ( wb_skip[ 2: 0] ==  3'b011_);
	assign	       bkram_sel = ( wb_skip[ 2: 2] ==  1'b1);
	assign	         sio_sel = ( wb_skip[ 2: 0] ==  3'b010_);
	assign	none_sel = (wb_stb)&&({ sio_sel, bkram_sel, uart_sel} == 0);
	//
	// many_sel
	//
...
`ifdef	VERILATOR
	...
	always @(*)
		case({sio_sel, bkram_sel, uart_sel})
			3'h0: many_sel = 1'b0;
			3'b100: many_sel = 1'b0;
			3'b010: many_sel = 1'b0;
			3'b001: many_sel = 1'b0;
...
`endif	// VERILATOR
	...
	always @(posedge i_clk)
		case({sio_ack, bkram_ack, uart_ack})
			3'h0: many_ack <= 1'b0;
			3'b100: many_ack <= 1'b0;
			3'b010: many_ack <= 1'b0;
			3'b001: many_ack <= 1'b0;
	...
	always @(posedge i_clk)
		wb_ack <= (wb_cyc)&&(|{sio_ack, bkram_ack, uart_ack});
...
`ifdef	BKRAM_ACCESS
	memdev #(.LGMEMSZ(11), .EXTRACLOCK(1))
		bkrami(i_clk,
			(wb_cyc), (wb_stb)&&(bkram_sel), wb_we,
				wb_addr[(11-3):0], wb_data, wb_sel,
...
`endif	// GPIO_ACCESS
	...
	//
	always @(posedge i_clk)
	casez({ buserr_sel, buspic_sel, gpio_sel, pwrcount_sel, version_sel })
		 5'b1????: sio_data <= buserr_data;
		 5'b01???: sio_data <= buspic_data;
		 5'b001??: sio_data <= gpio_data;
	...
	always @(posedge i_clk)
	begin
		casez({ sio_ack, bkram_ack, uart_ack })
			3'b1??: wb_idata <= sio_data;
			3'b01?: wb_idata <= bkram_data;
			3'b001: wb_idata <= uart_data;
