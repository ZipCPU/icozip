...
// Looking for string: SIM.TICK
		...
		// it here.
		if (m_core->o_spi_sck) {
			(*m_flash)(m_core->o_dspi_cs_n,
						(m_core->o_spi_sck)?0:1,
						m_core->o_spi_mosi);
		} m_core->i_miso = (*m_flash)(m_core->o_spi_cs_n, 1,
						m_core->o_spi_mosi);
#endif
...
// Looking for string: SIM.DEBUG
	...
	bool	load(uint32_t addr, const char *buf, uint32_t len) {
		...
		//
		// Loading the sram component
		//
		base  = 0x00c00000;
		naddr = 0x00008000;

		if ((addr >= base)&&(addr < base + naddr)) {
			// If the start access is in sram
...
#ifdef	SRAM_ACCESS
			// FROM sram.SIM.LOAD
			m_sram->load(start, &buf[offset], wlen);
